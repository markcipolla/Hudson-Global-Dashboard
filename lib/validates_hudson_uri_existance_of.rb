  ...
  def self.validates_hudson_uri_existence_of(*attr_names)
    ...
    validates_each(attr_names, configuration) do |r, a, v|
      if v.to_s =~ configuration[:with] # check RegExp
        ...
        begin #check it is a valid JSON response from a Hudson machine
          server_data = JSON.parse(Net::HTTP.get(URI.parse("#{URI.parse(v)}/api/json?depth=1")))
          server_data["jobs"].each do |job|
            job['name'] != ''
            job['color'] != ''
          end
        rescue
          r.errors.add(a, configuration_not_json[:message]) and false
        end
      else
        r.errors.add(a, configuration[:message]) and false
      end
